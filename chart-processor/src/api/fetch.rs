...
pub async fn fetch_single_make(origin: &str, make: &str) -> Result<Vec<ChartPoint>, String> {
    let url = format!(
        ...
        .filter_map(|car| {
            if let (Some(fuel), Some(range)) = (&car.fuel_tank_capacity, car.range_highway) {
                fuel.parse::<f32>().ok().and_then(|f| {
                    if f > 0.0 && range > 0.0 {
                        Some(ChartPoint {
                            x: (f * GALLONS_TO_LITERS).round(), // Convert gallons to liters
                            y: (range * MILES_TO_KM).round(),   // Convert miles to kilometers
                        })
                    } else {
                        None
                    }
